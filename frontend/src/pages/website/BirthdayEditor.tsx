...
export default function BirthdayEditor() {
  ...
  const [mainEventCategory, setMainEventCategory] = useState('Birthday');
  ...
  useEffect(() => {
    const fetchWebsite = async () => {
      try {
        ...
        const rawBirthday = rawContent.settings_json?.birthday || {};

        const defaultBirthday = {
          quote: rawBirthday.quote || rawContent.quote || 'Ready to celebrate an amazing milestone!',
          parentsName: rawBirthday.parentsName || 'Sarah & John',
          age: rawBirthday.age || '1',
          story_title: rawBirthday.story_title || rawContent.about_title || 'About The Party',
          mapUrl: rawBirthday.mapUrl || 'https://maps.app.goo.gl/Vg34LGmsU',
          contactNumbers: rawBirthday.contactNumbers || '9400850505, 403490349',
          countdownDate: rawBirthday.countdownDate || '2026-03-15T09:00',
          schedule: rawBirthday.schedule || [
            { event: "Cake Cutting", date: rawContent.date || "15 March 2026", time: "9:00 AM Onwards", venue: rawContent.contact_info?.address || "Kottakkal" },
            { event: "Dinner & Games", date: rawContent.date || "15 March 2026", time: "7:00 PM Onwards", venue: rawContent.contact_info?.address || "Kottakkal" }
          ],
          sections: rawBirthday.sections || [
            { id: 'hero', label: 'Cover / Hero', visible: true, locked: true },
            { id: 'about', label: 'Celebrant Details', visible: true },
            { id: 'story', label: 'About The Party', visible: true },
        ...
        const mergedContent = {
          ...
          about_title: rawContent.about_title || defaultBirthday.story_title,
          about_text: rawContent.about_text || 'Join us to celebrate this wonderful day!',
          contact_info: {
            address: 'Kottakkal',
        ...
        console.error(err);
        toast.error('Failed to load birthday invitation');
        navigate('/dashboard');
      } finally {
        setLoading(false);
      }
  ...
  const tabs = [
    ...
    { id: 'couple', icon: <Heart size={16} />, label: 'Celebrant' },
    { id: 'family', icon: <Users size={16} />, label: 'Parents / Hosts' },
  ...
  let rawSections = birthdayData.sections || defaultSections;
    ...
    <div className="flex h-screen bg-pink-50/20 font-sans overflow-hidden relative">
      ...
      <div className={`w-full lg:w-[420px] h-[calc(100vh-64px)] lg:h-screen mt-16 lg:mt-0 bg-white/70 backdrop-blur-2xl lg:border-r border-pink-100 shadow-xl flex flex-col relative z-20 ${mobileView === 'editor' ? 'flex' : 'hidden lg:flex'}`}>
        ...
        <div className="hidden lg:flex p-5 border-b border-pink-100 items-center justify-between">
          ...
          <h2 className="font-serif italic font-bold text-slate-800 text-xl truncate px-2">{content.hero_title || 'Birthday Invite'}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 scroll-smooth">
                      ...
                      <div className="flex items-center gap-3">
                        <button
                          onClick={() => {
                            ...
                            setBirthdayData({ sections: newSections });
                          }}
          ...
          {activeTab === 'couple' && (
            <div className="space-y-6 animate-in fade-in">
              <div className="bg-white p-5 rounded-2xl shadow-sm border border-pink-50 space-y-4">
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Celebrant Image</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Celebrant Name(s)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Turning Age</label>
                  <input
                    type="text"
                    value={birthdayData.age || ''}
                    onChange={(e) => setBirthdayData({ age: e.target.value })}
                    placeholder="e.g. 7 or 'Seven'"
                    className="w-full px-4 py-3 bg-slate-50 rounded-xl focus:ring-2 focus:ring-pink-500/20 outline-none font-medium"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Birthday Quote / Tagline</label>
                  <input
                    type="text"
                ...
                <div className="border-t border-slate-100 pt-4 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        type="text"
                        value={birthdayData.dateMonth || ''}
                        onChange={(e) => setBirthdayData({ dateMonth: e.target.value })}
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={birthdayData.dateDay || ''}
                        onChange={(e) => setBirthdayData({ dateDay: e.target.value })}
                        placeholder="22"
                        className="w-full px-3 py-2.5 bg-slate-50 rounded-xl focus:ring-2 focus:ring-pink-500/20 outline-none font-medium text-xs"
                      />
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      ...
                      <input
                        type="text"
                        value={birthdayData.dateWeekday || ''}
                        onChange={(e) => setBirthdayData({ dateWeekday: e.target.value })}
                        placeholder="MONDAY"
                        className="w-full px-3 py-2.5 bg-slate-50 rounded-xl focus:ring-2 focus:ring-pink-500/20 outline-none font-medium text-xs"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        type="text"
                        value={birthdayData.dateYear || ''}
                        onChange={(e) => setBirthdayData({ dateYear: e.target.value })}
                        placeholder="2030"
                        className="w-full px-3 py-2.5 bg-slate-50 rounded-xl focus:ring-2 focus:ring-pink-500/20 outline-none font-medium text-xs"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        type="text"
                        value={birthdayData.time || ''}
                        onChange={(e) => setBirthdayData({ time: e.target.value })}
                        placeholder="2:00 PM"
                        className="w-full px-3 py-2.5 bg-slate-50 rounded-xl focus:ring-2 focus:ring-pink-500/20 outline-none font-medium text-xs"
                      />
                    </div>
          ...
          {activeTab === 'family' && (
            <div className="space-y-6 animate-in fade-in">
              <div className="bg-white p-5 rounded-2xl shadow-sm border border-pink-50 space-y-6">
                <div className="space-y-3">
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-400">Parents & Family</label>
                  <input
                    type="text"
                    value={birthdayData.parentsName || ''}
                    onChange={(e) => setBirthdayData({ parentsName: e.target.value })}
                    placeholder="Mr. & Mrs. Smith (Smith Family)"
                    className="w-full px-4 py-3 bg-slate-50 rounded-xl focus:ring-2 focus:ring-pink-500/20 outline-none font-medium"
                  />
                  <div className="pt-2">
                    <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Host/Parents' Photo (URL or Upload)</label>
                    {birthdayData.parentsPhoto && (
                      <div className="mb-2 w-20 h-20 rounded-full overflow-hidden border-2 border-pink-100">
                        <img src={birthdayData.parentsPhoto} alt="Parents" className="w-full h-full object-cover" />
                      </div>
                    )}
                    <input
                      type="text"
                      value={birthdayData.parentsPhoto || ''}
                      onChange={(e) => setBirthdayData({ parentsPhoto: e.target.value })}
                      placeholder="https://example.com/parents.jpg"
                    ...
                    <FileUpload
                      accept="image/*"
                      label={birthdayData.parentsPhoto ? "Upload / Replace Photo" : "Upload Host/Parents' Photo"}
                      onChange={(url) => setBirthdayData({ parentsPhoto: url })}
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-6 animate-in fade-in">
              <div className="bg-white p-5 rounded-2xl shadow-sm border border-pink-50 space-y-4">
                <div>
                  ...
                  {(birthdayData.venuePhoto || content.venue?.image) && (
                    <div className="mb-3 h-36 rounded-xl overflow-hidden border border-pink-100 shadow-sm relative group">
                      <img src={birthdayData.venuePhoto || content.venue?.image} alt="Venue" className="w-full h-full object-cover" />
                    </div>
                  )}
                  <input
                    type="text"
                    value={birthdayData.venuePhoto ?? content.venue?.image ?? ''}
                    onChange={(e) => setBirthdayData({ venuePhoto: e.target.value })}
                  ...
                  <FileUpload
                    accept="image/*"
                    label={birthdayData.venuePhoto ? "Change Venue Photo" : "Upload Venue Photo"}
                    onChange={(url) => setBirthdayData({ venuePhoto: url })}
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={birthdayData.contactNumbers ?? ''}
                    onChange={(e) => setBirthdayData({ contactNumbers: e.target.value })}
            ...
            <div className="space-y-6 animate-in fade-in">
              <div className="bg-white p-5 rounded-2xl shadow-sm border border-pink-50 space-y-4">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <button
                    onClick={() => {
                      const currentGallery = birthdayData.gallery || [];
                      setBirthdayData({ gallery: [...currentGallery, ""] });
                    }}
                ...
                <div className="space-y-2">
                  <FileUpload
                    ...
                    onChange={(url) => {
                      const currentGallery = birthdayData.gallery || [];
                      setBirthdayData({ gallery: [...currentGallery, url] });
                    }}
                  />
                </div>

                <div className="space-y-3 pt-2">
                  {(birthdayData.gallery || []).map((url: string, index: number) => (
                    <div key={index} className="p-3 bg-slate-50 rounded-xl border border-slate-100 space-y-2">
                      <div className="flex gap-2 items-center">
                        ...
                        <input
                          ...
                          value={url}
                          onChange={e => {
                            const newGallery = [...(birthdayData.gallery || [])];
                            newGallery[index] = e.target.value;
                            setBirthdayData({ gallery: newGallery });
                          }}
                        ...
                        <button
                          onClick={() => {
                            const newGallery = (birthdayData.gallery || []).filter((_: any, i: number) => i !== index);
                            setBirthdayData({ gallery: newGallery });
                          }}
            ...
            <div className="space-y-6 animate-in fade-in">
              <div className="bg-white p-5 rounded-2xl shadow-sm border border-pink-50 space-y-4">
                <div>
                  ...
                  <input
                    type="text"
                    value={birthdayData.musicUrl || ''}
                    onChange={e => setBirthdayData({ musicUrl: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-50 rounded-xl focus:ring-2 focus:ring-pink-500/20 outline-none font-medium text-sm"
                  ...
                  <FileUpload
                    ...
                    onChange={(url) => setBirthdayData({ musicUrl: url })}
                  />
            ...
            <div className="space-y-6 animate-in fade-in">
              <div className="bg-white p-5 rounded-2xl shadow-sm border border-pink-50 space-y-4">
                <div>
                  ...
                  <input
                    type="datetime-local"
                    value={birthdayData.countdownDate || ''}
                    onChange={e => setBirthdayData({ countdownDate: e.target.value })}
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    <button
                      ...
                      onClick={() => {
                        ...
                        setBirthdayData({ countdownDate: isoStr });
                      }}
          ...
          {activeTab === 'schedule' && (() => {
            ...
            const scheduleList = (birthdayData.schedule && birthdayData.schedule.length > 0) ? birthdayData.schedule : defaultSchedule;
              ...
              <div className="space-y-6 animate-in fade-in">
                <div className="bg-white p-5 rounded-2xl shadow-sm border border-pink-50 space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <button
                      onClick={() => {
                        setBirthdayData({ schedule: [...scheduleList, { time: "12:00 PM", event: "New Event", date: content.date || "15 March 2026", venue: content.contact_info?.address || "Kottakkal" }] });
                      }}
                      ...
                      <div className="flex-1 space-y-2">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setBirthdayData({ schedule: newSched });
                          }}
