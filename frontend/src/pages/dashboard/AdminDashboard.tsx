...
import { Users, Globe, ShieldCheck, Printer, X, ShieldBan, Trash2, ExternalLink, Activity, DollarSign, TrendingUp, Search, UserMinus, ShieldAlert, CheckCircle2, ChevronRight, Menu, LogOut } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const handleToggleBlockUser = async (userId: number) => {
    if (userId === user?.id) return alert("You cannot block yourself.");
    try {
  ...
  const handleDeleteUser = async (userId: number) => {
    if (userId === user?.id) return alert("You cannot delete yourself.");
              ...
              {activeTab === 'websites' && (
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                  <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                     <button onClick={() => navigate('/dashboard')} className="bg-primary-600 hover:bg-primary-700 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-sm transition-colors flex items-center gap-2">
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-white border-b border-slate-200">
                          <th className="py-4 px-6 font-bold text-sm text-slate-500 uppercase tracking-wider">Website Info</th>
                          <th className="py-4 px-6 font-bold text-sm text-slate-500 uppercase tracking-wider">Status</th>
                          <th className="py-4 px-6 font-bold text-sm text-slate-500 uppercase tracking-wider text-center">Traffic</th>
                          <th className="py-4 px-6 font-bold text-sm text-slate-500 uppercase tracking-wider text-right">Actions</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100">
                        {filteredWebsites.map((w) => (
                          <tr key={w.id} className={`hover:bg-slate-50/50 transition-colors group ${w.is_blocked ? 'bg-rose-50/30' : ''}`}>
                            <td className="py-4 px-6">
                               <div className="font-black text-slate-900 text-lg flex items-center gap-2">
                                 {w.slug}
                                 {w.is_blocked && <ShieldAlert size={14} className="text-rose-500" />}
                               </div>
                               <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mt-1">{w.business_type} • {w.theme}</div>
                            </td>
                            <td className="py-4 px-6">
                              <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border ${w.published ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>
                                {w.published ? 'Live' : 'Draft'}
                              </span>
                            </td>
                            <td className="py-4 px-6 text-center">
                              <span className="font-black text-slate-700">{w.visitors_count || 0}</span>
                            </td>
                            <td className="py-4 px-6 text-right">
                              <div className="flex items-center justify-end gap-2">
                                ...
                                <button 
                                  onClick={() => handleToggleBlockWebsite(w.slug)}
                                  className={`p-2 rounded-lg transition-colors ${w.is_blocked ? 'text-rose-600 bg-rose-50 hover:bg-rose-100' : 'text-slate-400 hover:text-rose-600 hover:bg-rose-50'}`}
                                  title={w.is_blocked ? "Unblock/Restore Site" : "Suspend/Block Site"}
                                >
                                  <ShieldBan size={18} />
                                </button>
                                <button 
                                  onClick={() => handleDeleteWebsite(w.slug)}
                                  className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors"
                                  title="Delete Site"
                                >
                                  <Trash2 size={18} />
