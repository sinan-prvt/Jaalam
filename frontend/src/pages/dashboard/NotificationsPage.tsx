...
export default function NotificationsPage({ isAdminView = false }: NotificationsPageProps) {
  ...
  const handleMarkAsRead = async (id: number) => {
    try {
      const endpoint = user?.is_superuser 
        ? `/api/users/notifications/${id}/mark_read/?all=true`
  ...
  const handleSendReply = async () => {
    ...
    try {
      const endpoint = isAdminView 
        ? `/api/users/notifications/${selectedNotification.id}/reply/?all=true`
        : `/api/users/notifications/${selectedNotification.id}/reply/`;
