...
class ArcballControl {
  ...
  constructor(canvas: HTMLCanvasElement, updateCallback?: (deltaTime: number) => void) {
    ...
    canvas.addEventListener('pointerdown', this.#pointerDownHandler as any);
    canvas.addEventListener('pointerup', this.#pointerUpHandler);
    canvas.addEventListener('pointerleave', this.#pointerLeaveHandler);
    canvas.addEventListener('pointermove', this.#pointerMoveHandler as any);
  ...
  destroy() {
    this.canvas.removeEventListener('pointerdown', this.#pointerDownHandler as any);
    this.canvas.removeEventListener('pointerup', this.#pointerUpHandler);
    this.canvas.removeEventListener('pointerleave', this.#pointerLeaveHandler);
    this.canvas.removeEventListener('pointermove', this.#pointerMoveHandler as any);
...
class InfiniteGridMenu {
  ...
  destroy() {
    ...
    if (gl) {
      try {
        ...
        console.error('Error destroying WebGL context:', e);
      }
    }
  }
        ...
        item =>
          new Promise<HTMLImageElement>(resolve => {
            ...
            img.onerror = () => {
              ...
              // Fallback to a dark-gray 1x1 data URL so layout doesn't freeze
              const fallbackCanvas = document.createElement('canvas');
              fallbackCanvas.width = 1;
              fallbackCanvas.height = 1;
              const fallbackCtx = fallbackCanvas.getContext('2d');
              if (fallbackCtx) {
                fallbackCtx.fillStyle = '#27272a';
  ...
  #render() {
    const gl = this.gl;
    if (!this.discVAO || !this.discProgram) return;
  ...
  #findNearestVertexIndex(): number | undefined {
  ...
  return (
    <div className="relative w-full h-full">
      <canvas
        ...
        ref={canvasRef}
        className="cursor-grab w-full h-full overflow-hidden relative outline-none active:cursor-grabbing [touch-action:none]"
      />
        ...
        <>
          <h2
            className={`
              ...
              ${
                isMoving
                  ? 'opacity-0 duration-[100ms] -translate-y-2 scale-95 md:translate-y-[-60%] md:scale-100'
                  : 'opacity-100 duration-[500ms] translate-y-0 scale-100 md:-translate-y-1/2'
              }
            `}
          >
            {activeItem.title}
          ...
          <p
            className={`
              ...
              ${
                isMoving
                  ? 'opacity-0 duration-[100ms] -translate-x-1/2 translate-y-2 md:translate-x-0 md:translate-y-[-40%]'
                  : 'opacity-100 duration-[500ms] -translate-x-1/2 translate-y-0 md:translate-x-0 md:-translate-y-1/2'
              }
            `}
          >
            ...
            className={`
              ...
              ${
                isMoving
                  ...
                  : 'bottom-6 md:bottom-[3.8em] opacity-100 pointer-events-auto duration-[500ms] scale-100 -translate-x-1/2'
              }
            `}
          >
            <p className="select-none relative text-[#120F17] top-[1px] md:top-[2px] text-[22px] md:text-[26px]">&#x2197;</p>
          </div>
        </>
      )}
    </div>
